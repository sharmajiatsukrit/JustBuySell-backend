...
import { Sessions, User, Customer } from "../../../models";
...
async function fetchSessionlogin(sessionData: SessionFetchData): Promise<SessionManageData> {
    try {
        const dbSession = await Sessions.findOne({ id: sessionData.session_id }).lean();

        const userData = await Customer.findOne({ id: sessionData.user_id }).lean();
        if (!dbSession || !userData) throw new Error("SESSIONNOTFOUND");
...
export { fetchSession, fetchSessionlogin };
